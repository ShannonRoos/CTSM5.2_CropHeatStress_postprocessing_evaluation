...
def cft_to_crops(ds, time_dim='year'):
    ''' Merge cfts to get global crop maps'''
    
    crop_map = {
        'spring_wheat': ['spring_wheat'],
        'corn': ['temperate_corn', 'tropical_corn'],
        'rice': ['rice'],
        'soybean': ['soybean', 'tropical_soybean'],
        'irrigated_spring_wheat': ['irrigated_spring_wheat'],
        'irrigated_corn': ['irrigated_temperate_corn', 'irrigated_tropical_corn'],
        'irrigated_rice': ['irrigated_rice'],
        'irrigated_soybean': ['irrigated_soybean', 'irrigated_tropical_soybean'],
    }
    ...
    for var, da in ds.data_vars.items():
        ...
        for crop, cfts in crop_map.items():
            out = da.sel(cft=cfts[0])

            if len(cfts) == 2:
                alt = da.sel(cft=cfts[1])
                mask = out.isnull().all(dim=time_dim)
                out = out.where(~mask, alt)

            crop_arrays.append(out.reset_coords(drop=True))

        ds_out[var] = xr.concat(crop_arrays, dim="crop").assign_coords(
            crop=list(crop_map)
        )
...
def standardized_yield_anomalies(ds_detrend, T_dim='year'):
    '''
...
def global_production_yield(area_grid_m2, grainc_gCm2_grid, Y1,Yn, crop_list,y_ranges=0, plot='off'):
    ...
    ***** Built on bit's and pieces from Sam's code (2023: https://doi.org/10.5194/gmd-16-7253-2023) *****
    ...
    #set 0 to nan to not mess with averages
    area_grid_m2 = area_grid_m2.where(area_grid_m2>0.0)
    grainc_gCm2_grid = grainc_gCm2_grid.where(area_grid_m2>0.0)
    ...
    #set 0 to nan to not mess with averages
    area_grouped         = area_grouped.where(area_grouped>0.0)
    productiongC_grouped = productiongC_grouped.where(area_grouped>0.0)
